...
let sortRanges range1 range2 = 
    if fst range1 > fst range2 then
        range2, range1
    else 
        range1, range2

let rangeIntersects range1 range2 =
    let range1, range2 = sortRanges range1 range2
...
let intersects cuboid1 cuboid2 =
    ...
    let xIntersect = rangeIntersects cuboid1.XRange cuboid2.XRange
    let yIntersect = rangeIntersects cuboid1.YRange cuboid2.YRange
    let zIntersect = rangeIntersects cuboid1.ZRange cuboid2.ZRange
    xIntersect && yIntersect && zIntersect

let rangeIntersection range1 range2 =
    let range1, range2 = sortRanges range1 range2

    let intersectionStart = fst range2
    let intersectionEnd = snd range1

    intersectionStart, intersectionEnd

let intersection cuboid1 cuboid2 = 
    
    let xIntersection = rangeIntersection cuboid1.XRange cuboid2.XRange
    let yIntersection = rangeIntersection cuboid1.YRange cuboid2.YRange
    let zIntersection = rangeIntersection cuboid1.ZRange cuboid2.ZRange

    { XRange = xIntersection; YRange = yIntersection; ZRange = zIntersection }
    
let secondStar = 
    let rebootSteps = parse example3 |> Seq.map snd |> List.ofSeq

    intersection rebootSteps.[0] rebootSteps.[1]
    // rebootSteps
    // |> Seq.map (fun rebootStep ->
    //     let intersectsWith =
    //         rebootSteps
    //         |> Seq.filter (fun other -> other <> rebootStep)
    //         |> Seq.filter(fun other ->
    //             intersects other rebootStep
    //         )
    //         |> Seq.toList
    //     rebootStep, intersectsWith
    // )
    // |> Seq.toList
